...
//$today = date('z');
$noofdays = ($today - $refdate)/86400 + 1;
...
foreach($rooms as $room)
{
	...
	for($i = 0; $i < $noofdays; $i++)
	{
		...
		" INTERVAL $i DAY AND time < '2014-04-22' + INTERVAL ".($i+1).
		" DAY AND DAYOFWEEK(time) != 1 AND DAYOFWEEK(time) !=7";
		$result = mysqli_query($db,$sql);
	...
	for($j = 1; $j < 4; $j++)
	{
		echo $sql2 = 'UPDATE days_control_mode_weekdays SET `mode_'.$j.'` = '.
...
for($j = 1; $j < 4; $j++)
{
	$sql2 = 'UPDATE days_control_mode_weekdays SET `mode_'.$j.'` = '.
...
echo "<br><br>calculate_mode_days_weekdays.php end <br>";		
?>